...
void solve_flow(vd1 & x, vd1 & y, vd2 & u, vd2 & v, vd2 & p){
    // 書き込みファイルの作成
    std::ofstream MAC_simple_1;
    MAC_simple_1.open("MAC_example_1000.csv", std::ios::trunc);
    std::ofstream MAC_simple_2;
    MAC_simple_2.open("MAC_example_2000.csv", std::ios::trunc);
    std::ofstream MAC_simple_3;
    MAC_simple_3.open("MAC_example_3000.csv", std::ios::trunc);
    std::ofstream MAC_simple_4;
    MAC_simple_4.open("MAC_example_4000.csv", std::ios::trunc);
    std::ofstream MAC_simple_5;
    MAC_simple_5.open("MAC_example_5000.csv", std::ios::trunc);
    std::ofstream MAC_simple_omega;
    MAC_simple_omega.open("MAC_example_omega.csv", std::ios::trunc);
    std::ofstream rhs_hs;
    ...
    for (int n=1; n<=nlast; n++){
        ...
        for (int j=j_1; j<=j_2-1; j++){
            double cpfore = (2.0*p[i_1][j]+2.0*p[i_1][j+1])/2.0;
        ...
        for (int i=i_1; i<=i_2-1; i++){
            double cpbtm = (2.0*p[i][j_1]+2.0*p[i+1][j_1])/2.0;
            double cptop = (2.0*p[i][j_2]+2.0*p[i+1][j_2])/2.0;
            cl = cl+(cpbtm-cptop)*dx;
        }

        double cp1 = 2.0*p[i_2+i_2-i_1][j_1];
        double cp2 = 2.0*p[i_2+i_2-i_1][j_2];
        ...
        if (n == 1000){
            for (int i=1; i<=mx; i++){
                for (int j=1; j<=my; j++){
                    MAC_simple_1 << x[i] << "," << y[j] << "," << 2.0*p[i][j] << std::endl; // pの値を2倍することに注意
                }
                MAC_simple_1 << std::endl; // gnuplotのpm3dのために必要
            }
        }
        ...
        if (n == 2000){
            for (int i=1; i<=mx; i++){
                for (int j=1; j<=my; j++){
                    MAC_simple_2 << x[i] << "," << y[j] << "," << 2.0*p[i][j] << std::endl; // pの値を2倍することに注意
                }
                MAC_simple_2 << std::endl; // gnuplotのpm3dのために必要
            }
        }
        ...
        if (n == 3000){
            for (int i=1; i<=mx; i++){
                for (int j=1; j<=my; j++){
                    MAC_simple_3 << x[i] << "," << y[j] << "," << 2.0*p[i][j] << std::endl; // pの値を2倍することに注意
                }
                MAC_simple_3 << std::endl; // gnuplotのpm3dのために必要
            }
        }
        ...
        if (n == 4000){
            for (int i=1; i<=mx; i++){
                for (int j=1; j<=my; j++){
                    MAC_simple_4 << x[i] << "," << y[j] << "," << 2.0*p[i][j] << std::endl; // pの値を2倍することに注意
                }
                MAC_simple_4 << std::endl; // gnuplotのpm3dのために必要
            }
        }
        ...
        if (n == nlast){
            for (int i=1; i<=mx; i++){
                for (int j=1; j<=my; j++){
                    MAC_simple_5 << x[i] << "," << y[j] << "," << 2.0*p[i][j] << std::endl; // pの値を2倍することに注意
                    MAC_simple_omega << x[i] << "," << y[j] << "," << omega[i][j] << std::endl;
                }
                MAC_simple_5 << std::endl; // gnuplotのpm3dのために必要
                MAC_simple_omega << std::endl; // gnuplotのpm3dのために必要
            }
        }
    }
