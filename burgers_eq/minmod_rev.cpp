...
double dsign(double value, double sign);
...
int main(){
    ...
    u_muscl.open("u_muscl_minmod_copy.csv", std::ios::trunc);
...
double dsign(double value, double sign){
    return (sign >= 0.0) ? std::abs(value) : -std::abs(value);
    }
...
void reconst(double u[], double uL[], double uR[]){
    for (int i=1; i<=imax; i++){
        ...
        double dul_lim = dsign(1.0, dul)*std::max(0.0, std::min(std::abs(dul), dsign(1.0, dul)*dur*b));
        double dur_lim = dsign(1.0, dur)*std::max(0.0, std::min(std::abs(dur), dsign(1.0, dur)*dul*b));
