...
double superbee(double d1, double d2, double delta, double omega);
...
int main(){
    ...
    for (int n=1; n<=nmax; n++){
        ...
        if (n==int(nmax/2)){
            ...
            u_trans.open("u_muscl_superbee_trans.csv", std::ios::trunc);
    ...
    u_muscl.open("u_muscl_superbee_final.csv", std::ios::trunc);
...
double superbee(double d1, double d2, double delta, double omega){
    ...
    if (d1*d2<=0){
        ...
        if (d1/d2<=0.5){
            ...
            zeta = std::min(zetaR, d1/d2);
            zeta = std::min(zeta, 2.0);
        }
    }
    return zeta*delta;
}
