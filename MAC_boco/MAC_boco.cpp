...
// 凹型になるように物体を設定(もう少し計算格子を細くした方がいいかも)
...
void poiseq(vd2 & u, vd2 & v, vd2 & p){
    ...
    for (int i=2; i<=mx-1; i++){
        for (int j=2; j<=my-1; j++){
            // 物体内部は計算しない
            if (i>i_1 && i<i_3 && j>j_1 && j<j_2){
                continue;
            }
            else if (i>i_2 && i<i_3 && j>=j_2 && j<=j_3){
                continue;
            }
            else if (i>i_1 && i<i_3 && j>j_3 && j<j_4){
                continue;
            }
...
void solve_flow(vd1 & x, vd1 & y, vd2 & u, vd2 & v, vd2 & p){
    // 書き込みファイルの作成
    std::ofstream MAC_boco_1;
    MAC_boco_1.open("MAC_boco_1000.csv", std::ios::trunc);
    std::ofstream MAC_boco_2;
    MAC_boco_2.open("MAC_boco_2000.csv", std::ios::trunc);
    std::ofstream MAC_boco_3;
    MAC_boco_3.open("MAC_boco_3000.csv", std::ios::trunc);
    std::ofstream MAC_boco_4;
    MAC_boco_4.open("MAC_boco_4000.csv", std::ios::trunc);
    std::ofstream MAC_boco_5;
    MAC_boco_5.open("MAC_boco_5000.csv", std::ios::trunc);
    std::ofstream MAC_boco_omega;
    MAC_boco_omega.open("MAC_boco_omega.csv", std::ios::trunc);
    std::ofstream rhs_hs;
    ...
    for (int n=1; n<=nlast; n++){
        ...
        for (int j=j_3; j<=j_4-1; j++){
            double cpfore3 = (2.0*p[i_1][j]+2.0*p[i_1][j+1])/2.0;
            cd = cd+cpfore3*dy;
        }
        ...
        for (int i=i_1; i<=i_2-1; i++){
            double cptop1 = (2.0*p[i][j_2]+2.0*p[i+1][j_2])/2.0;
            double cpbtm1 = (2.0*p[i][j_3]+2.0*p[i+1][j_3])/2.0;
            cl = cl+(cpbtm1-cptop1)*dx;
        }
        for (int i=i_1; i<=i_3-1; i++){
        ...
        if (n == 1000){
            for (int i=1; i<=mx; i++){
                for (int j=1; j<=my; j++){
                    MAC_boco_1 << x[i] << "," << y[j] << "," << 2.0*p[i][j] << std::endl; // pの値を2倍することに注意
                }
                MAC_boco_1 << std::endl; // gnuplotのpm3dのために必要
            }
        }
        ...
        if (n == 2000){
            for (int i=1; i<=mx; i++){
                for (int j=1; j<=my; j++){
                    MAC_boco_2 << x[i] << "," << y[j] << "," << 2.0*p[i][j] << std::endl; // pの値を2倍することに注意
                }
                MAC_boco_2 << std::endl; // gnuplotのpm3dのために必要
            }
        }
        ...
        if (n == 3000){
            for (int i=1; i<=mx; i++){
                for (int j=1; j<=my; j++){
                    MAC_boco_3 << x[i] << "," << y[j] << "," << 2.0*p[i][j] << std::endl; // pの値を2倍することに注意
                }
                MAC_boco_3 << std::endl; // gnuplotのpm3dのために必要
            }
        }
        ...
        if (n == 4000){
            for (int i=1; i<=mx; i++){
                for (int j=1; j<=my; j++){
                    MAC_boco_4 << x[i] << "," << y[j] << "," << 2.0*p[i][j] << std::endl; // pの値を2倍することに注意
                }
                MAC_boco_4 << std::endl; // gnuplotのpm3dのために必要
            }
        }
        ...
        if (n == nlast){
            for (int i=1; i<=mx; i++){
                for (int j=1; j<=my; j++){
                    MAC_boco_5 << x[i] << "," << y[j] << "," << 2.0*p[i][j] << std::endl; // pの値を2倍することに注意
                    MAC_boco_omega << x[i] << "," << y[j] << "," << omega[i][j] << std::endl;
                }
                MAC_boco_5 << std::endl; // gnuplotのpm3dのために必要
                MAC_boco_omega << std::endl; // gnuplotのpm3dのために必要
            }
        }
    }
